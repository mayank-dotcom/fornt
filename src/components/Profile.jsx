...
function Profile() {
  ...
  // Function to convert months to hours using dailyHours from schema
  const convertMonthsToHours = (durationString, dailyHoursString) => {
    // Extract the number of months from the duration string (e.g., "3 months" -> 3)
    const months = parseInt(durationString);
    // Extract the number of daily hours from the dailyHours string (e.g., "8 hours" -> 8)
    const dailyHours = parseInt(dailyHoursString);
  ...
  const fetchInternDetails = async () => {
    try {
      ...
      // Calculate total required hours using both internshipDuration and dailyHours from schema
      const requiredHours = convertMonthsToHours(
        details.internshipDuration,
        details.dailyHours
      );
      setTotalRequiredHours(requiredHours);
  ...
  // Calculate progress based on dynamic total required hours
  const progress = totalRequiredHours > 0 ? (totalVerifiedDuration / totalRequiredHours) * 100 : 0;
    ...
    <div className="card h-100">
      <div className="card-body d-flex flex-column">
        ...
        <div className="mt-3">
          ...
          <div className="mt-4">
            <h5 className="mb-2">
              Progress: {totalVerifiedDuration} out of {totalRequiredHours} required hours
            </h5>
            <div className="progress">
              <div
                className="progress-bar"
